...
namespace ProcessingModule
{
    ...
    public class Acquisitor : IDisposable
    {
        ...
        /// <summary>
        /// Acquisitor thread logic - fires every second, reads config items based on their acquisition interval.
        /// </summary>
		private void Acquisition_DoWork()
        {
            while (true)
            {
                // Timer thread signals this every 1 second
                ...
                foreach (IConfigItem configItem in configuration.GetConfigurationItems())
                {
                    ...
                    if (configItem.SecondsPassedSinceLastPoll >= configItem.AcquisitionInterval)
                    {
                        ...
                        {
                            stateUpdater.LogMessage($"{ex.TargetSite.ReflectedType.Name}.{ex.TargetSite.Name}: {ex.Message}");
                        }
                    }
                }
            }
